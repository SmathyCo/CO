    ...
    if (data) {
        ...
        for (let i = 0; i < dataNextLine.length; i++) {
            for (let i2 = 0; i2 < dataNextLine[i].length; i2++) {
                ...
                let secondBrace = dataNextLine[i][i2].indexOf("}"); // Getting where is the ending brace

                if (endParen > startParen && endParen < secondBrace) { // Don't tokenize the code inside functions
